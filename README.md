...
- 仪表盘（含可调时间窗口）
- 一键 JSON 导入导出
- 单文件 exe（控制台 + 自动开浏览器 tab）

**v0.2（当前）** ✅
- 改造为 Windows 原生桌面应用（PyWebView，无黑窗、无浏览器 tab）
- 单实例 + 拉前台（再次双击图标召回已有窗口）
- 记忆窗口大小与位置
- 数据自动落地 LOCALAPPDATA + 便携模式 fallback
- 透明底自定义图标
- UI 修复：URL 可点、题目不再折叠、Markdown 长内容不溢出
